...
name: sem
description: >
  Entity-level semantic diff for Git that tracks functions, classes, methods, and types
  instead of lines. Uses tree-sitter and AST-normalized structural hashing to detect
  cosmetic vs. structural changes, perform rename detection, and analyze cross-file
  dependencies across 27+ languages. Bridges the gap between Git's line-based tracking
  and developer mental models.
